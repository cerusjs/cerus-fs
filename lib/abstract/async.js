...
class stats {
	...
	is() {
		if(this._is === undefined) {
			this._is = new is(this._cerus, this._path, this._obj);
		}
...
class is {
	...
	blockdevice() {
		return get_stat("isBlockDevice", this._path.path, this._cerus, this._obj, true);
	}

	characterdevice() {
		return get_stat("isCharacterDevice", this._path.path, this._cerus, this._obj, true);
	}

	directory() {
		return get_stat("isDirectory", this._path.path, this._cerus, this._obj, true);
	}

	fifo() {
		return get_stat("isFIFO", this._path.path, this._cerus, this._obj, true);
	}

	file() {
		return get_stat("isFile", this._path.path, this._cerus, this._obj, true);
	}

	socket() {
		return get_stat("isSocket", this._path.path, this._cerus, this._obj, true);
	}

	link() {
		return get_stat("isSymbolicLink", this._path.path, this._cerus, this._obj, true, "lstat");
	}
}

var get_stat = function(key, path, cerus, obj, func = false, type = "stat") {
	return cerus.promise(function(event) {
		fs[type](path, function(err, stats) {
			if(err) {
				return event("error");
			}
