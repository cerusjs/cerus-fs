...
class abstract_sync {
	...
	exists() {
		return fs.existsSync(this._path.path);
	}
	...
	stats() {
		if(this._stats === undefined) {
			this._stats = new stats(this._path);
		}
...
class is {
	...
	blockdevice() {
		return fs.statSync(this._path.path).isBlockDevice();
	}

	characterdevice() {
		return fs.statSync(this._path.path).isCharacterDevice();
	}

	directory() {
		return fs.statSync(this._path.path).isDirectory();	
	}

	fifo() {
		return fs.statSync(this._path.path).isFIFO();
	}

	file() {
		return fs.statSync(this._path.path).isFile();
	}

	socket() {
		return fs.statSync(this._path.path).isSocket();
	}

	link() {
		return fs.lstatSync(this._path.path).isSymbolicLink();
	}
}